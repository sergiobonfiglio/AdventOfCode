...
--TODO: rewrite using Seq
--import           Data.Sequence
import           Debug.Trace

solve = show (last outputs)
  where
    (_, _, _, outputs) = next' (0, inputTape, 1, [])

solvePart2 = show (last outputs)
  where
    (_, _, _, outputs) = next' (0, inputTape, 5, [])

next' :: (Int, [Int], Int, [Int]) -> (Int, [Int], Int, [Int])
next' (pos, tape, input, out)
  | pos < 0 || pos >= length tape = (pos, tape, input, out)
  | otherwise = next' (nextPos, nextTape, nextInt, nextOut)
  where
    (nextPos, nextTape, nextInt, nextOut) = next (pos, tape, input, out)
...
next (posIn, tapeIn, input, outputs)
  ...
  --inputs
  | opcode == 3 = (posIn + 2, updateTape val1 input, param1, outputs)
  --outputs
  ...
  where
    ...
    paramVal pos mode =
      if mode == 0
        then tapeIn !! (tapeIn !! pos) --position mode
